describe('The author field', function() {
    ...
    describe('For unknown authors', function() {
        it('should not be possible to activate the author', function() {
            ...
            cy.get('#first_name_1').as('first')
                .should('be.empty')
                .should('not.be.readonly');

            cy.get('#last_name_1').as('last')
                .should('be.empty')
                .should('not.be.readonly')
                .type('bohr');

            cy.get('.modal-dialog').as('modal')
            ...
            cy.get('@modal')
                .should('be.visible')
                .should('contain', 'No matching entry in staff directory found. ' +
    ...
    describe('For authors without an old name', function() {
        it('should be possible to activate the author', function() {
            ...
            cy.get('@first')
                .should('have.value', 'Albert')
                .should('be.readonly');

            cy.get('@last')
                .should('have.value', 'Einstein')
                .should('be.readonly');
        });
    });
    ...
    describe('For authors with an old name', function() {
        it('should be possible to activate the author by current name', function() {
            ...
            cy.get('@first')
                .should('have.value', 'Marilyn')
                .should('be.readonly');

            cy.get('@last')
                .should('have.value', 'Monroe')
                .should('be.readonly');
        });
        ...
        it('should be possible to activate the author by old name', function() {
            ...
            cy.get('@first')
                .should('have.value', 'Norma Jeane')
                .should('be.readonly');

            cy.get('@last')
                .should('have.value', 'Mortenson')
                .should('be.readonly');
        });
    });
});
