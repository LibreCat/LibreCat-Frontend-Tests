    ...
    describe('The Display / Sort options', function () {
      describe('The Hits per page option', function () {
        ...
        it('should open when button is clicked', function () {
          ...
          cy.get('@hits-per-page-collpasible').should('be.visible').find('a:contains("100")').click()

          cy.url().should('match', /record\?limit=100/)

          cy.get('@hits-per-page-button').should('have.text', `${this.t.facets.hits_per_page}: 100`)
        })

        it('should display the number of marked publications', function () {
          cy.intercept('/marked_total*', {
            total: 1234,
            ok: 1,
          })
          ...
          cy.get('.total-marked').should('be.visible').should('have.text', '1234')
        })
      })
    })
  })
